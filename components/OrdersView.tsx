...
import { 
  ...
  Phone, CheckCircle2, 
  RefreshCw, AlertCircle, ShoppingCart, Loader2
} from 'lucide-react';
import { OrderData } from '../types';
import { fetchAvailability } from '../services/sheetsService';
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 animate-[fadeIn_0.5s_ease-out]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        ...
        {!showForm && !isSuccess && (
          <div className="flex items-center gap-4">
             {isLoadingStock && (
               <div className="flex items-center gap-2 text-[9px] font-gamer text-[#00f2ff] animate-pulse">
                  <Loader2 className="w-3 h-3 animate-spin" /> Atualizando Estoque...
               </div>
             )}
            <button 
              onClick={() => setShowForm(true)}
              className="flex items-center gap-2 px-5 py-2.5 bg-white/5 border border-white/10 rounded-lg text-[9px] font-gamer text-white hover:bg-white/10 transition-all uppercase tracking-[0.2em]"
            >
              <ShoppingCart className="w-3.5 h-3.5" /> Outra Encomenda
            </button>
          </div>
        )}
      </div>
        ...
        <div className="space-y-12">
          ...
          <div className="bg-[#0a0a0c]/80 backdrop-blur-xl border border-[#39ff14]/20 rounded-2xl p-6 md:p-8 shadow-xl relative overflow-hidden group max-w-5xl mx-auto">
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
              ...
              <div>
                <p className="text-sm md:text-base text-gray-300 font-medium leading-relaxed italic opacity-90">
                  "O botão Encomendas garante a você acesso prioritário aos itens desejados. Registre seu interesse antecipadamente e, assim que o produto estiver disponível em nosso estoque, você terá preferência na compra. Nossa equipe entrará em contato para negociar condições exclusivas e apresentar uma oferta personalizada."
                </p>
              </div>
            </div>
          </div>
